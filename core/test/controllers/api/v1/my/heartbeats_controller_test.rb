...
class Api::V1::My::HeartbeatsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should get heartbeat and update last_active_at" do
    assert_changes -> { @session.reload.last_active_at } do
      get api_v1_my_heartbeat_url, headers: { Authorization: "Bearer #{@token}" }
    end
  ...
  test "should return unauthorized without token" do
    get api_v1_my_heartbeat_url
    assert_response :unauthorized
  end

  test "should return unauthorized with invalid token" do
    get api_v1_my_heartbeat_url, headers: { Authorization: "Bearer invalid_token" }
    assert_response :unauthorized
  end
end
