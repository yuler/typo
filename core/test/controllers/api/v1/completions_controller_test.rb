...
class Api::V1::CompletionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should get success on create" do
    post api_v1_completions_url, params: { text: "__fake_test_text__", prompt: "correct this" }, as: :json
  ...
  test "should rate limit after 5 requests" do
    5.times do
      post api_v1_completions_url, params: { text: "__fake_test_text__" }, as: :json
      assert_response :success
    end
  ...
  test "authenticated users should bypass rate limit" do
    ...
    6.times do
      post api_v1_completions_url, params: { text: "__fake_test_text__" }, headers: { "Authorization" => "Bearer #{token}" }, as: :json
      assert_response :success
    end
  end
end
