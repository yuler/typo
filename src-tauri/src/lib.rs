...
async fn process_text(text: String) -> Result<(), String> {
    ...
    enigo.key(enigo::Key::Escape, enigo::Direction::Press).unwrap();
    enigo.key(enigo::Key::Escape, enigo::Direction::Release).unwrap();
    
    // Type each character individually to avoid IME
    for c in text.chars() {
        if c.is_ascii() {
            enigo.key(enigo::Key::Unicode(c), enigo::Direction::Press).unwrap();
            enigo.key(enigo::Key::Unicode(c), enigo::Direction::Release).unwrap();
        }
    }
