...
Use this skill to open a **new** pull request whose title and body are derived from the current branch. The body follows `gh-pr-summary` structure (a built-in Claude Code harness skill); the title is a single plain sentence rolled up from every commit in the range.
...
3. **Choose base branch**
   ...
   - **Note**: These commands assume `origin` is the remote name. If using a different remote (e.g., in a fork), replace `origin` with your remote name.
