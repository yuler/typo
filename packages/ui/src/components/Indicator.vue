      ...
      >
        <div v-if="state === 'processing'" class="flex max-w-full items-center gap-2 px-2 overflow-hidden" :data-tauri-drag-region="embedded ? undefined : true">
          <div v-if="commandName" class="flex items-center gap-1 shrink-0 bg-blue-500/10 pl-1 pr-1.5 py-0.5 rounded border border-blue-500/20" :data-tauri-drag-region="embedded ? undefined : true">
            ...
            <span class="text-[10px] font-bold text-blue-400 tracking-tight uppercase" :data-tauri-drag-region="embedded ? undefined : true">
              {{ commandName.startsWith('/') ? commandName.slice(1) : commandName }}
            </span>
          </div>
          <Loader2Icon class="w-3.5 h-3.5 animate-spin text-blue-400 shrink-0" :data-tauri-drag-region="embedded ? undefined : true" />
          <span class="truncate text-sm text-blue-100/90 shrink min-w-0 font-medium" :data-tauri-drag-region="embedded ? undefined : true">{{ inputText }}</span>
          <span class="text-[10px] text-blue-400/40 font-mono shrink-0" :data-tauri-drag-region="embedded ? undefined : true">{{ inputText?.length }}</span>
        </div>

        <div v-else-if="state === 'result'" class="flex items-center gap-2 px-2 overflow-hidden" :data-tauri-drag-region="embedded ? undefined : true">
          <span class="truncate text-sm text-green-400 font-medium" :data-tauri-drag-region="embedded ? undefined : true">{{ resultText }}</span>
          <template v-if="copyResult">
            <ClipboardCheckIcon class="w-4 h-4 text-green-400 shrink-0" :data-tauri-drag-region="embedded ? undefined : true" />
            <span class="text-[10px] text-green-400/50 font-mono shrink-0" :data-tauri-drag-region="embedded ? undefined : true">{{ labels?.copied ?? 'COPIED' }}</span>
          </template>
        </div>
        ...
        <p
          ...
          :data-tauri-drag-region="embedded || isRateLimited ? undefined : true"
        ...
        <kbd v-else class="px-1.5 py-0.5 rounded border border-white/10 bg-white/5 font-mono text-[10px] text-white/40" :data-tauri-drag-region="embedded ? undefined : true">
